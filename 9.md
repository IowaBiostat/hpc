...
cluster goes at the end (if you specify `-b y`). Now this command means that
we've submitted 10 jobs to run on 10 different nodes, each one opening a
separate container to run our simulation.

If you look at the one of the log files (e.g., `apptainer.o6279783.5`), you'll
see that the R version matches that of the container, not the system.
