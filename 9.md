...
A slightly less flexible, but far more convenient alternative, `qsub` itself supports basic looping at the command line with what it calls *array jobs*.  Essentially, `qsub` sets up a loop like the one above for you.  Since each run of `sim.R` already saves to a uniquely-named file (via the random hash), our `sim` script needs no modification:
...
{% capture _code %}q_sub -t 1-15 sim{% endcapture %}
{% include prompt.html code=_code %}

This will submit 15 independent runs of `sim.R`, each saving its own `sim-[hash].rds` file.  This is very convenient as it lets you change the number of jobs on the fly from the command line.
...
{% capture _code %}q_sub -tc 2 -t 1-15 sim{% endcapture %}
{% include prompt.html code=_code %}
