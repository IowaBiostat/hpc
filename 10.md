...
Rscript sim.R
{% endcapture %}
{% include file.html name="batch.txt" code=_code %}

Then we can submit this as an array job with 4 cores (one per line) via:

{% capture _code %}qbatch -q BIOSTAT batch.txt{% endcapture %}
...
`qbatch` is most useful when your commands are not all identical --- for example, when running several different analysis scripts, or running the same script with different input files.  For pure replications like the simulation above, array jobs (previous page) are simpler.

If you want to have each job run more than one line, you can change the "chunk size":

{% capture _code %}qbatch -q BIOSTAT -c 2 batch.txt{% endcapture %}
{% include prompt.html code=_code %}
...
{% capture _code %}qbatch -q BIOSTAT -c 2 --ppj 2 batch.txt{% endcapture %}
{% include prompt.html code=_code %}
...
{% capture _code %}qbatch -q BIOSTAT -c 2 --ppj 2 -j 2 batch.txt{% endcapture %}
{% include prompt.html code=_code %}
...
{% capture _code %}qbatch -q BIOSTAT -- ls{% endcapture %}
{% include prompt.html code=_code %}
