...
## How to manage queue usage

To manage the use of queues, there are a number of things you can do, depending on your situation and workload. Here are a few different strategies:

1. Submit appropriately sized jobs in chunks, moving on to the next when your previous job has finished.
2. Use the `at` command to schedule the submission of jobs for off hours.
3. Use the `-tc` flag with the `qsub` command to control how many indices from an array job are allowed to simultaneously execute. 
