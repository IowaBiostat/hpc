...
If, on the machine you're using, your username is not the same as your HawkID, you'll have to specify your login username:
...
- [VS Code](https://code.visualstudio.com/) with the [Remote-SSH extension](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-ssh). Once connected, VS Code lets you browse and edit files on Argon directly and provides an integrated terminal.
- WSL (as described above)
- [Git Bash](https://git-scm.com/downloads) is a lighter-weight alternative that provides a bash shell and ssh without installing a full Linux distribution.
- [PuTTY](http://www.chiark.greenend.org.uk/~sgtatham/putty/) is also capable as a standalone graphical SSH client that requires no installation.

For all of these options, specify `argon.hpc.uiowa.edu` as the host, port 22 on-campus (or 40 off-campus), and connection type SSH.
