...
**Note:** Be aware when mapping a drive in this way that you will be opening and editing files on a Windows machine that must be readable on to the HPC Linux machines. In particular, Windows and Linux files have different line endings, and this will cause problems depending on what program you use to edit files. If at any point you see error messages like `bin/bash^M: bad interpreter: No such file or directory`, or anything with a `^M` in it, this is likely what has happened.

To fix this in VS Code, click the `CRLF` indicator in the bottom-right status bar and select `LF`. To make LF the default for all new files, add `"files.eol": "\n"` to your VS Code settings. If you're using Notepad++, go to Edit > EOL Conversion > Unix (LF) for an existing file, or Settings > Preferences > New Document > Unix (LF) to set the default.
</div>
...
# Transferring files (Linux/Mac)

## scp

The standard way to transfer files from a Linux/Mac terminal is `scp`:
...
Alternatively, you can use cloud storage for file transfer using, say, GitHub or Amazon Web Services: from your local machine, push to the cloud, then pull down from argon. If you know how `git` works, then this is self-explanatory; if you don't, well, [that's a separate tutorial](http://gitimmersion.com).
