...
<span style="color:red">This content is depricated, and may not work. For up to date versions of R, we recommend using containers. The information is currently left here for context, and may be removed in the future.  </span>
...
{% endcapture %}
{% include prompt.html more="$<br/>$ <br/>$" code=_code %}
...
module load curl
{% endcapture %}
{% include prompt.html more="$<br/>$<br/>$<br/>$" code=_code %}
...
{% capture _code %}mkdir ~/buildR && cd ~/buildR{% endcapture %}
{% include prompt.html code=_code %}
...
{% capture _code %}tar -xzvf $RVERS.tar.gz{% endcapture %}
{% include prompt.html code=_code %}
...
./configure
{% endcapture %}
{% include prompt.html more="$" code=_code %}
...
{% capture _code %}make -j 2{% endcapture %}
{% include prompt.html code=_code %}
...
{% endcapture %}
{% include prompt.html more="$<br/>$<br/>$<br/>$<br/>$<br/>$<br/>$<br/>$<br/>$<br/>$<br/>$" code=_code %}
